...
function add_tmm_scripts() { wp_enqueue_style( 'tmm', plugins_url('css/tmm_custom_style.css', __FILE__)); }


// Enqueue admin styles
add_action( 'admin_enqueue_scripts', 'add_admin_tmm_style' );
function add_admin_tmm_style() { wp_enqueue_style( 'tmm', plugins_url('css/admin_de_style.css', __FILE__)); }
            ...
            $output .= '<div class="tmm_'.$tmm_columns.'_columns tmm_wrap '.$ori_f.'">';
                ...
                if (is_array($members) || is_object($members))
                    foreach ($members as $key => $member) {
                        ...
                        if ($key == $page_count - 1)$output .= '<div class="clear" style="clear:both;"></div>';
                    }
